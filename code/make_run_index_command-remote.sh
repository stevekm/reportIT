...
find /results/analysis/output/Home -name "*.xls" -name "R_*" -path "*variantCaller_out.*" ! -name "*cov.xls" | while read file; do make_barcodes_list "$file" | grep -Ev '^#' | grep -Ev '^$'; done
