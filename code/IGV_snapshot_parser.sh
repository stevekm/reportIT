...
for i in $analysis_samples; do
    ...
    if (( $num_lines > $min_number_lines )); then 
        ...
        $IGV_batchscript_generator_script "$sample_summary_file" "$sample_bamfile" "$sample_IGV_dir"
    ...
    # RUN IGV ON THE BATCHSCRIPT
    # $IGV_run_batchscript_script
done
...
# code/IGV_batchscript_generator.py output/.../IonXpress_002_summary.tsv output/....bam output/.../IonXpress_002/IGV_snapshots -cb /path/to/control.bam
