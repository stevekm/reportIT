...
for ID in $analysis_ID; do
    ...
    qsub -wd $PWD -o :${analysis_logdir}/ -e :${analysis_logdir}/ -pe threaded "$job_threads" -l mem_free="$job_mem" -l h_vmem="$job_mem" -l mem_token="$job_mem" -N "$job_name" $job_options "$annotate_wrapper_script" "$ID"
done
